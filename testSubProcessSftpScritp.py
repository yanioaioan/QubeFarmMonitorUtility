	...
	if flagcontinue==True:
			...
			for file in glob.glob(mylocalpath+str('*')):
				...
				if os.path.isdir(file):
					...
					os.system("sftp "+username+"@tete <<< mkdir myremotepath+base+str('/')")
					#sftp.chdir(myremotepath+base+str('/'))
					os.system("sftp "+username+"@tete <<< cd myremotepath+base+str('/') ")
					
					#recall uploadAllFilesToSFTP() for the newly found dir that we 're in now
					uploadAllFilesToSFTP(file+str('/*'), myremotepath+base+str('/'), "sftp", "False", str(username))
				else:
